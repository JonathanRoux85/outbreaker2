...
BurninChange <- function(res, burning, init_alpha, ids, real_data, min.support, include_imported){
...
ParametersSynthesis_1run <- function(results, burning,
  ...
  if(NewBurning){
    results <- BurninChange(res = results$results_mcmc$res,
                            real_data = chains_detect100_bind,
                            min.support = min.support,
                            burning = burning,
                            init_alpha = results$results_mcmc$res_consensus$init_alpha,
                            ids = results$results_mcmc$res_consensus$to_label,
                            include_imported = include_imported)
  }
  ...
  ######
  # Sensitvity #
  se_links.aa <- results$parameters$parameters_links_aa$se_links.aa
  
  # Specificity #
  sp_links.aa <- results$parameters$parameters_links_aa$sp_links.aa
  
  # Number of links correctly retrieved #
  tp_links.aa <- results$parameters$parameters_links_aa$tp_links.aa
  
  # Positive predictive value #
  ppv_links.aa <- results$parameters$parameters_links_aa$ppv_links.aa

  # Negative predictive value #
  npv_links.aa <- results$parameters$parameters_links_aa$npv_links.aa
  
  # Percentage of entire chains reconstructed #
  global_chains_aa <- results$parameters$parameters_chains_aa$global_chains_aa
  ...
  ######
  # Maximum sensitivity #
  se_links.aa_max <- max(results$parameters$parameters_links_aa$se_links.aa)

  # ID of the maximum sensitivity #
  se_max_id <- which(results$parameters$parameters_links_aa$se_links.aa == 
                       max(results$parameters$parameters_links_aa$se_links.aa))[1]

  
  # Specificity related to maximum sensitivity #
  sp_links.aa_related_se <- results$parameters$parameters_links_aa$sp_links.aa[se_max_id]
  
  # Number of links correctly retrieved related to maximum sensitivity #
  tp_links.aa_related_se <- results$parameters$parameters_links_aa$tp_links.aa[se_max_id]

  
  # Minimal support related to maximum sensitivity #
  minimal_support.aa_related_se <- results$parameters$minimal_support[se_max_id]
  
  # Positive predictive value retrieved related to maximum sensitivity #
  ppv_links.aa_related_se <- results$parameters$parameters_links_aa$ppv_links.aa[se_max_id]
  
  # Negative predictive value related to maximum sensitivity #
  npv_links.aa_related_se <- results$parameters$parameters_links_aa$npv_links.aa[se_max_id]
  
  # Percentage of entire chains reconstructed related to maximum sensitivity #
  global_chains_aa_related_se <- results$parameters$parameters_chains_aa$global_chains_aa[se_max_id]
  
  ######
  ### Youden index ###
  ######
  # Youden index #
  youden_index <- results$parameters$parameters_links_aa$se_links.aa +
    results$parameters$parameters_links_aa$sp_links.aa -
    1
  
  # Index of Youden index max #
  youden_max <- which.max(results$parameters$parameters_links_aa$se_links.aa +
                            results$parameters$parameters_links_aa$sp_links.aa -
                            1)

  # Sensitivity linked to Youden max #
  se_youden <- results$parameters$parameters_links_aa$se_links.aa[youden_max]
  
  # Specificity linked to Youden max #
  sp_youden <- results$parameters$parameters_links_aa$sp_links.aa[youden_max]
  
  # Number of links correctly retrieved related to Youden max #
  tp_youden <- results$parameters$parameters_links_aa$tp_links.aa[youden_max]
  
  # Positive predictive value retrieved related to Youden max #
  ppv_youden <- results$parameters$parameters_links_aa$ppv_links.aa[youden_max]

  # Negative predictive value related to Youden max #
  npv_youden <- results$parameters$parameters_links_aa$npv_links.aa[youden_max]
  
  # Minimal support related to Youden max #
  minimal_support_youden <- results$parameters$minimal_support[youden_max]

  # Percentage of entire chains reconstructed related to Youden max #
  global_chains_youden <- results$parameters$parameters_chains_aa$global_chains_aa[youden_max]
  ...
  output <- data.table(pi = ParametersEditing(pi,2,FALSE),
                       ...
                       youden_index_max = round(youden_max,0),
                       minimal_support_youden = round(minimal_support_youden,4),
                       se_youden = round(se_youden*100,2),
                       sp_youden = round(sp_youden*100,2),
                       tp_youden = round(tp_youden,2),
                       ppv_youden = round(ppv_youden*100,2),
                       npv_youden = round(npv_youden*100,2),
                       global_chains_youden = round(global_chains_youden*100,2)
  )
  ...
  ###########################
  # Preparation of data #
  data_ROC <- data.table(sp = results$parameters$parameters_links_aa$sp_links.aa, 
                         se = results$parameters$parameters_links_aa$se_links.aa,
                         min.support = min.support)
  ...
  FigPlot <- ggplot(data = data_ROC, aes(x = 1-sp, y = se)) +
    geom_line() +
    geom_point() +
    theme_minimal() +
    xlab("1-Specificity") +
    ...
    geom_segment(aes(x = 1 - data_ROC[no_line == youden_max, sp] + 0.015, 
                     xend = 1 - data_ROC[no_line == youden_max, sp] - 0.015,
                     y = data_ROC[no_line == youden_max, se] - 0.02, 
                     yend = data_ROC[no_line == youden_max, se] + 0.02), 
  ...
  list_output <- list(parameters = output,
                      max_links_retrieved = results$parameters$max_links_retrieved,
                      ROC = FigPlot)
  ...
  ######################################
  if(NotRetrieved){
    links_not_retrieved <- merge(results$results_mcmc$res_aa,
    ...
    ## Complete the output ##
    list_output$not_retrieved_links <- from_to.dt[, scenario := 1]
  }
  ...
  ################################################
  if(StudyKappa){
    kappa_new_links <- merge(results$results_mcmc$res_aa,
